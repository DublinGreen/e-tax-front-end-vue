...
<v-container grid-list-lg class="dashboard-wrapper">
  <div class="headline mb-5 font-weight-medium">Quick Pay</div>
  ...
  <v-sheet class="px-4 pt-4 pb-5 my-shadow-2" style="opacity: 0.9">
    ...
    <v-layout row wrap>
      <v-flex xs12 sm12 md4>
        ...
        <v-autocomplete 
          ...
          name="mda"
          v-validate="'required'"
          @change="getRevenueSource(bill.agency_code)" :class="{ grayBox: isMDA }" :disabled="isMDA">
          <div slot="label">{{bill.agency_code ? '' : 'Choose'}}</div>
        </v-autocomplete>
      ...
      <v-flex xs12 sm6 md4>
        ...
        <v-select 
          ...
          name="revenue source"
          v-validate="'required'">
          <div slot="label">{{bill.revenue_code ? '' : 'Choose'}}</div>
        </v-select>
    ...
    <paye :model_="bill.payment_breakdown" 
      v-if="bill.bill_category === 'PAYE'" 
      @generateBill="generateBill"/>
    
    <dta :model_="bill.payment_breakdown" 
      v-if="bill.bill_category === 'DTA'" 
      @generateBill="generateBill"/>

    <others :model_="bill.payment_breakdown" 
      v-if="bill.bill_category === 'OTHERS'" 
      @generateBill="generateBill"/>
    ...
    getMda(){
      this.$axios.get('MDAs/get-mdas')
          .then(response => {
            ...
            const mdaData = localStorage.getItem('_dk2ldd23kj2'); // fetch user_role as saved in the login page
            /* let's confirm if current logged account is an MDA
            * and then we try to auto select
            * an MDA sector for this user... easier UX
            * */
            (mdaData && mdaData === 'MDA') ? (this.mdas.length === 1) ? (this.bill.agency_code = this.mdas[0],this.isMDA = true) : '' : '';
          })
          .catch(error => {
