    ...
    <v-flex md12 sm12 mt-4>
      ...
      <v-data-table hide-actions :headers="headers" :items="receipts">
        <template v-slot:items="props">
          <td>{{ props.item.receipt_number }}</td>
          ...
          <td>{{ props.item.date_issued | moment("YYYY-MM-DD") }}</td>
          <td class="text-xs-center">
            <v-layout>
              <v-btn flat small class="primary action-btn" @click="viewReceipt(props.item)">View</v-btn>
            </v-layout>
          </td>
        </template>
      </v-data-table>
...
export default {
  ...
  methods: {
    searchReceipts(pageUrl) {
        ...
        this.$axios.$get('bill-payments/search', {params: this.paramsModel}).then(res => {
