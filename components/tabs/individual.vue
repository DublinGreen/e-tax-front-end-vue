  ...
  <v-sheet class="px-4 py-5 my-shadow-2" style="opacity: 0.9">
    <v-layout row wrap>
      ...
      <v-flex xs12 sm6 md4>
        ...
        <v-text-field name="email" v-model="searchModel.params.email" type="email" outline v-validate="{ required: false, regex: /^\w+([.-]?\w+)*@\w+([.-]?\w+)*(\.\w{2,8})+$/ }">
      ...
      <v-flex xs12 sm6 md4>
        ...
        <v-text-field name="phone number" v-model="searchModel.params.phone" type="tel" outline v-validate="{ required: false, regex: /^([0-9]{11})$/ }">
          <div class="" slot="label">Phone Number</div>
        </v-text-field>
        <div class="error-box">{{ errors.first('phone number') }}</div>
      </v-flex>
