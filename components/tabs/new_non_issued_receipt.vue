...
<section>
  <v-layout row wrap>
    <v-flex v-if="runSearch" md12 sm12 mt-4>
      ...
      <v-flex xs12 sm6 md4 :pr-2="this.$vuetify.breakpoint.smAndUp">
        <div class="caption font-weight-bold mb-1">Bill Reference Code</div>
        <v-text-field v-model="searchModel.params.bill_reference_code" outline>
          <div class="" slot="label">Search by Bill reference code</div>
        </v-text-field>
      </v-flex>
      ...
      <v-data-table v-if="runSearch || showTable" hide-actions :headers="headers" :items="receipts">
        <template v-slot:items="props">
          ...
          <td>{{ props.item.bill_reference_code }}</td>
          <td>{{ props.item.anssid }}</td>
          <td>{{ props.item.anssid }}</td>
          <td>{{ props.item.amount | formatNumber }}</td>
          <td class="text-xs-center">
            <v-layout>
              <v-btn flat small class="primary action-btn" @click="issueReceipt(props.item.payment_reference)">
                ISSUE
              </v-btn>
            </v-layout>
          </td>
...
export default {
  ...
  methods: {
    searchReceipts(pageUrl) {
        ...
        this.$axios.$get('bill-payments/search-non-issued', {params: this.paramsModel}).then(res => {
    ...
    getNonIssuedReceipts(pageUrl = null) {
        this.loading = true;
        let url = "bill-payments/non-issued";
        if (pageUrl != null) {
          url = pageUrl;
        }
    ...
    issueReceipt(paymentReference) {
      ...
      this.$axios.$post('bill-payments/issue-receipt',  this.issueReceiptModel)
      .then(res => {
